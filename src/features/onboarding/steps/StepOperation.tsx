...
import { listBlueprints } from "@/modules/bioterio/species/data/blueprintRegistry";
...
export default function StepOperation() {
  ...
  const isBlueprintSelected = (taxonomyKey: string) =>
    speciesSeed?.kind === "blueprint" && speciesSeed.taxonomyKey === taxonomyKey;
  const isCustomSelected = speciesSeed?.kind === "custom";
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Elige el Blueprint que dictará las reglas de tu entorno.
        </p>
      </div>
                ...
                {selected && opt.id === "Bioterio" && (
                  <div className="mt-6 pt-4 border-t border-border/50 animate-in slide-in-from-top-4 fade-in duration-300">
                    ...
                    <div className="grid gap-2 grid-cols-2 md:grid-cols-4 lg:grid-cols-5">
                      {blueprints.map((b) => {
                        const selected = isBlueprintSelected(b.taxonomyKey);
                        return (
                          <div
                            key={b.taxonomyKey}
                            onClick={(e) => {
                              e.stopPropagation();
                              selectBlueprintSpecies(b.taxonomyKey, b.displayName);
                            }}
                            className={`p-2.5 rounded-md border text-sm cursor-pointer transition-colors flex items-center justify-between gap-2
                              ${selected ? "border-primary bg-primary/10 text-primary font-medium" : "border-border bg-background hover:border-primary/40 text-muted-foreground"}
                            `}
                          >
                      ...
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
                          selectCustomSpecies(customSpeciesText);
                        }}
                        className={`p-2.5 rounded-md border text-sm cursor-pointer transition-colors flex items-center justify-between gap-2
                          ${isCustomSelected ? "border-primary bg-primary/10 text-primary font-medium" : "border-border bg-background hover:border-primary/40 text-muted-foreground"}
                        `}
                      >
                        <span className="truncate">Personalizada</span>
                        {isCustomSelected && <Check className="h-4 w-4 shrink-0" />}
                      </div>
                    </div>

                    {isCustomSelected && (
                      <div className="mt-4 max-w-sm animate-in fade-in duration-200" onClick={(e) => e.stopPropagation()}>
                        <Label htmlFor="custom-species" className="text-xs mb-1.5 block">
                          Nombre de la especie / raza
                        </Label>
                        <Input
                          id="custom-species"
                          value={customSpeciesText}
                          onChange={(e) => selectCustomSpecies(e.target.value)}
