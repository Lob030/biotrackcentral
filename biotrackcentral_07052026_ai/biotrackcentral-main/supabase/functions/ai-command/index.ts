...
async function recordTelemetry(sb: SupabaseClient, orgId: string, userId: string, eventType: string, durationMs?: number, metadata?: Record<string, unknown>) {
  try {
...
async function actionParse(req: Request) {
  ...
  for (const r of rejected) {
    invalid.push({ id: `tmp-${r.index + 1}`, error: r.reason, raw: r.raw });
    await recordTelemetry(sb, orgId, userId, "semantic_failure", undefined, { reason: r.reason, raw: r.raw });
  }
  ...
  for (const op of normalized) {
    ...
    if (res.ok) {
      valid.push(res.op);
      if (res.op.intent === "requires_clarification") {
         await recordTelemetry(sb, orgId, userId, "clarification_triggered", undefined, { 
             missing_fields: res.op.payload.missing_fields,
             ambiguous_references: res.op.payload.ambiguous_references,
             razon: res.op.payload.razon
         });
      }
    } else {
      invalid.push(res.bad);
      await recordTelemetry(sb, orgId, userId, "semantic_failure", undefined, { error: res.bad.error, raw: res.bad.raw });
    }
  }

  if (invalid.length > 0) {
    console.warn("[ai-command] parse produced", invalid.length, "invalid op(s)");
  } else if (valid.every(v => v.intent !== "requires_clarification")) {
    await recordTelemetry(sb, orgId, userId, "parse_success", undefined, { intent_count: valid.length });
  }
...
async function actionTelemetry(req: Request) {
  ...
  await recordTelemetry(sb, orgId, userId, event_type, duration_ms, metadata);
  return json({ ok: true });
}
